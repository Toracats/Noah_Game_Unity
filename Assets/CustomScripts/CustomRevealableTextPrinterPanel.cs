...
namespace Naninovel.UI
{
    ...
    /// </remarks>
    public class CustomRevealableTextPrinterPanel : UITextPrinterPanel, ILocalizableUI
    {
        [Serializable]
        ...
        protected virtual void SetMessageAuthor (MessageAuthor author)
        {
            ...
            // CustomCharacterManager にキャスト
            var customCharacterManager = CharacterManager as CustomCharacterManager;
            if (customCharacterManager != null && AffilicationPanel) // AffilicationPanel の null チェックも追加
            {
                var affiliation = customCharacterManager.GetAffiliation(Author.Id);
                SetAffiliationText(affiliation); // 新しいメソッドを呼び出す
                // 必要であれば、所属情報のテキストカラーも設定できます
                // AffilicationPanel.TextColor = ...; 
            }
